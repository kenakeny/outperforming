"""horizon_2week — same ensemble, but a 10-trading-day (~2 week) forward label.

Every model so far predicts 5-day forward peer-relative excess. Longer
horizons average out more short-term noise, which might make DIRECTION
(the thing we're trying to fix) easier to call correctly, at the cost of a
less responsive signal. Features are unchanged (they're all trailing price
features, independent of label horizon) -- only the label and embargo change.

Trains the same two-stage ensemble (extreme-gate + direction) on a 1-year
window, tests on 2026 only. Reports the same metrics as the 5-day version
(argmax confusion matrix, flip rates, gated win-rate/payoff at a few K) so
it's a direct apples-to-apples comparison against the 5-day results already
in hand.
"""
import sys, pathlib
...
print(f"2-week horizon, 1y window: fit={fit_m.sum()} val={val_m.sum()} "
...
# baseline multiclass, for the confusion-matrix / flip-rate comparison
with gpu_lock():
...
cm = confusion_matrix(y_te, pred_base, labels=[0, 1, 2])
under_over = cm[0, 2] / cm[0].sum(); over_under = cm[2, 0] / cm[2].sum()
print(f"\nbaseline 2-week: acc={accuracy_score(y_te, pred_base):.4f}  "
     f"f1={f1_score(y_te, pred_base, average='macro'):.4f}  "
     f"under->over={under_over:.3f}  over->under={over_under:.3f}")
...
print(f"\nensemble 2-week: tail_auc={tail_auc:.4f}")

is_ext = p_ext >= 0.5
pred_ens = np.where(~is_ext, 1, np.where(p_dir >= 0.5, 2, 0))
cm2 = confusion_matrix(y_te, pred_ens, labels=[0, 1, 2])
u2o = cm2[0, 2] / cm2[0].sum(); o2u = cm2[2, 0] / cm2[2].sum()
print(f"ensemble argmax: acc={accuracy_score(y_te, pred_ens):.4f}  "
     f"under->over={u2o:.3f}  over->under={o2u:.3f}")
print(pd.DataFrame(cm2, index=MC_NAMES, columns=MC_NAMES).to_string())
...
print("\n===== 2-week horizon selectivity results =====")
print(res.to_string(index=False))
res.to_csv(EXP / "results" / "horizon_2week.csv", index=False)
print("\nsaved -> experiments/results/horizon_2week.csv")

"""
Train and save a model on the universe-wide label.
...
comparable across every fund and can drive a cross-sectional portfolio.
    ...
    python -m scripts.data.train_universe
    python -m scripts.data.train_universe --kind catboost --horizon 5
"""
import argparse
import pathlib
...
def main():
    ...
    if hasattr(m, "save_model"):
        ...
        joblib.dump(m, out)
    print(f"saved -> {out}")
    print(f"\nbacktest it:\n  python -m scripts.evaluation.backtest_vs_spy {out} "

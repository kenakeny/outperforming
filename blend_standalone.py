"""
blend_standalone.py -- ETF outperformance model, everything in one file.
...
them yourself with the included build_dataset.py (free, no API key):
  ...
  data/raw/metadata.parquet      one row per ticker, must have a 'category' column
    ...
    python blend_standalone.py                    # full run
    python blend_standalone.py --no-gnn           # XGBoost only, no torch needed
    python blend_standalone.py --data-dir path/   # if parquets live elsewhere
